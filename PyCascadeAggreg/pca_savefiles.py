...
def save_index(date_ex: str, csv_index_file: str, agg_index: str, dataset_name: str, agg_method_layer_one: str, agg_method_layer_two: str, outlayer: str, top_k: int, top_m: int, top_m_lt_type: str, top_m_lt: int, alpha: float , effectiveness_mode: str,  l_size: int, map_result: float, run_time: float):

    with open(csv_index_file, 'a', newline='') as file:
        writer = csv.writer(file, delimiter=";")
        writer.writerow([date_ex ,agg_index, dataset_name, agg_method_layer_one, agg_method_layer_two, outlayer, top_k, top_m, top_m_lt_type, top_m_lt, alpha, effectiveness_mode, l_size, map_result, run_time]) 
    
    # Fecha o arquivo após escrever o cabeçalho
    file.close()
    return