...
def paths_creations(dataset_name: str, mode: str):
    ...
    rootDir = os.getcwd()
    output_path = f"{rootDir}/output/{dataset_name}"
    csv_index_file = f"{rootDir}/{dataset_name}_index.csv"
    ...
    output_rk_fusion_path = f"{output_dataset_path}/rk_fusions"
    ...
    if not os.path.exists(csv_index_file):
        ...
        with open(csv_index_file, 'w', newline='') as file:
            ...
            writer.writerow(['Date','Index','DataSet', 'Layer One', 'Layer Two', 'Outlayer', "Top K", "Top M","Top M Layer Two type", "Top M Layer Two","Alpha", "Effectiveness", "L Size", "MAP", 'Execution Time (s)']) 
