...
def plot_dot_graph(output_dataset_path: str, dataset_name: str, top_k: int, top_m: int): 
    ...
    ax1.axhline(y=best_MAP, color='red', linestyle='--', label="Best isolated ranker")
    # Configurações do gráfico...
    ax1.legend()
    ax1.set_xlabel("Borda Score", fontsize=24)
    ax1.set_ylabel("MAP", fontsize=24)
    ax1.set_title("Dots graph - MAP vs Borda Score")
    ax1.grid(True)

    # Ajustando o tamanho da fonte nos valores dos eixos
    ax1.tick_params(axis='x', labelsize=25)  # Tamanho da fonte no eixo x
    ax1.tick_params(axis='y', labelsize=25)  # Tamanho da fonte no eixo y
    ...
    # Caso contrário, se o top_k for menor ou igual a 7, manter uma coluna normal na legenda
        ...
        ax2.set_title('Rankers',verticalalignment='center', fontsize=40)
...
def plot_surf_graph(dataset_name:str):

    root_dir = os.getcwd()

    # Carregar o arquivo CSV
    dados = pd.read_csv(f'{root_dir}/{dataset_name}_index.csv', delimiter=';')
    dados = dados.sort_values(by='Top K')
    dados = dados.sort_values(by='Beta')
    ...
    ax.plot_trisurf(top_k, beta, mapa, cmap='viridis', edgecolor='none')
    ...
    # Mostrar o gráfico
    #plt.show()
    plt.savefig(f'{root_dir}/{dataset_name}_surf.png')
